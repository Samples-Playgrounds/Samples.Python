...
import api_cunking_splitting_by_characters_with_overlap as api
...
#   # OCR => slow
#   # f"pymupdf4llm-c",
#   # f"deepdoctection-pytorch",
#   # f"deepdoctection-tensorflow",

pdf_extracted_texts = [
                  f"text/python/pymupdf4llm/content.md",
                  f"text/python/pdfplumber/content.txt",
                  f"text/python/markitdown/content.md",
                  f"text/python/docling/content.txt",
                  f"text/python/docling/content.md",
                  f"text/python/docling/content.filetype-agnostic.md",
                  f"text/python/PyMuPDF_fitz/content.txt",
                  f"text/python/PyPDF2/content.txt",
                  f"text/python/pdfminer_six/content.txt",
                  f"text/python/unstructured/content.txt",
                  f"text/python/marker/content.md",
                  f"text/python/pypdfium2/content.txt",
                  f"text/python/pytesseract/content.txt",
]

docx_extracted_texts = [
                  f"text/python/pymupdf4llm/content.md",
                  f"text/python/docx2txt/content.txt",
                  f"text/python/markitdown/content.md",
                  f"text/python/docling/content.txt",
                  f"text/python/docling/content.md",
                  f"text/python/docling/content.filetype-agnostic.md",
                  f"text/python/docx/content.txt",
                  f"text/python/docx2python/content.txt",
]
...
sources = sources_sports_book + sources_pravno + sources_books_microsoft
...
texts_chonkie = [
   """
   Chonkie is the goodest boi! My favorite chunking hippo hehe.
   """,
]
texts_chunklet = [
   """
...
def test_text():
   ...
   for text in texts:
      print(120 * "")
      result_chunks = api.text_split_chunks_on_character_count_with_overlap(text, chunk_size)
...
def test_files():
   ...
   for source in sources:
      ...
      if source.endswith(".pdf"):
         ...
         for part_pdf in pdf_extracted_texts:
            file = f"{folder}/{part_pdf}"
            print(f"    Processing txt file: {file}")

            if (file is None) or (not os.path.isfile(file)):
               print(f"Error not found {file}")
               continue

            result_chunks_file = api.file_split_chunks_on_character_count_with_overlap(file)

            with open(file,"r") as f:
               text_to_split = f.read()
   
            result_chunks = api.text_split_chunks_on_character_count_with_overlap(text_to_split)

            json_str = json.dumps(result_chunks)
            folder_results = f"{folder}/{part_pdf}.hwaifs/splitting-chunks/characters-by-characters-with-overlap/"
            Path(folder_results).mkdir(parents=True, exist_ok=True)
            file = f"{folder_results}/chunks-size.json"
      ...
      if source.endswith(('.docx', '.doc')) :
         ...
         for part_docx in docx_extracted_texts:
            file = f"{folder}/{part_pdf}"
            print(f"    Processing txt file: {file}")
            ...
            json_str = json.dumps(result_chunks)
            folder_results = f"{folder}/{part_docx}.hwaifs/splitting-chunks/characters-by-characters-with-overlap/"
            Path(folder_results).mkdir(parents=True, exist_ok=True)
            file = f"{folder_results}/chunks-size.json"
            with open(file, "w") as final:
	            json.dump(json_str, final)
