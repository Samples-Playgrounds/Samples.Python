...
import api_cunking_splitting_by_words_with_overlap as api
...
def test_text():
   ...
   for text in texts:
      print(120 * "")
      result_chunks = api.text_split_chunks_on_word_count_with_overlap(text, chunk_size)
   ...
   for source in sources:
      ...
      if source.endswith(".pdf"):
         ...
         for part_pdf in pdf_extracted_texts:
            ...
            result_chunks_file = api.file_split_chunks_on_word_count_with_overlap(file)

            with open(file,"r") as f:
               text_to_split = f.read()

            result_chunks = api.text_split_chunks_on_word_count_with_overlap(text_to_split)

            json_str = json.dumps(result_chunks)
            folder_results = f"{folder}/{part_pdf}.hwaifs/splitting-chunks/characters-by-word-with-overlap/"
      ...
      if source.endswith(('.docx', '.doc')) :
         ...
         for part_docx in docx_extracted_texts:
            ...
            json_str = json.dumps(result_chunks)
            folder_results = f"{folder}/{part_docx}.hwaifs/splitting-chunks/characters-by-word-with-overlap/"
            Path(folder_results).mkdir(parents=True, exist_ok=True)
            file = f"{folder_results}/chunks-size.json"
            with open(file, "w") as final:
	            json.dump(json_str, final)
