...
Sets background color, using name or hex value <body text=?> Sets text color, using name or hex value 
...
Inserts a horizontal rule <hr size=?> Sets size (height) of horizontal rule 
...
Adds image; it is a separate file located at the URL <img src="URL" align=?> Aligns image left/right/center/bottom/top/middle (use CSS) <img src="URL" border=?> Sets size of border surrounding image (use CSS) 
...
**Tables** (use only for data layout - use CSS for page layout) **Table attributes** (only use for email newsletters) <table> </table> <table border=?> 
