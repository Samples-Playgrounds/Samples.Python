...
Nested matching and bind- ing are also allowed.
...
```
anyChoice1 ch = Just (First _)  "First!" Just Second  "Second!"
```

```
case ch of Nothing  "No choice!" _  "Something else."
```
...
instance Flavor Bool where avor \_ = "sweet" instance Flavor Char where avor \_ = "sour" Evaluating flavor True gives: &gt; avor True "sweet" While flavor 'x' gives: &gt; avor 'x' "sour"
...
```
class Eq a where () :: a  a  Bool () :: a  a  Bool () a b =  (a  b)
```
...
MyType is the type's name . MyValue1 and MyValue are values of the type and are called constructors. Multiple constructors are separated with the '|' character. Note that type and constructor names must start with a capital letter. It is a syntax error otherwise.
...
```
| Uncon { conValue :: String } | Noncon whichCon con = "convalue is " ++ conValue con
```
...
The do keyword indicates that the code to follow will be in a monadic context. Statements are separated by newlines, assignment is indicated by &lt;-, and a let form is introduce which does not require the in keyword.
...
```
wrong leName = if doesFileExist leName then ... else ...
```
...
```
right1 leName = do   doesFileExist leName if  then return 1 else return 0
```
...
```
let result = if  then 1 else 0 return result
```
...
```
countBytes1 f = do putStrLn "Enter a lename." args  getLine if length args  0 -- no 'do'. then putStrLn "No lename given." else -- multiple statements require -- a new 'do'. do f  readFile args putStrLn ("The le is " ++ show (length f) ++ " bytes long.")
```
...
```
countBytes3 = do putStrLn "Enter a lename." args  getLine case args of []  putStrLn "No args given." le  do { f  readFile le; putStrLn ("The le is " ++ show (length f) ++ " bytes long."); }
```
...
```
-- Use pattern-matching to -- get first character sentenceCase (s:rest) = if isLower s then toUpper s : rest else s : rest
```
...
let "functions" with no arguments are actually constants and, once evaluated, will not evaluate again. This is useful for capturing common portions of your function and re-using them. Here is a silly example which gives the sum of a list of numbers, their average, and their median:

```
listStats m = let numbers = [1,3 .. m] total = sum numbers mid = head (take (m `div` 2) numbers) in "total: " ++ show total ++ ", mid: " ++ show mid
```
...
```
rstThree str = let (a:b:c:_) = str in "Initial three characters are: " ++ show a ++ ", " ++ show b ++ ", and " ++ show c
```

Note that this is different than the following, which only works if the string has three characters:

```
onlyThree str = let (a:b:c:[]) = str in "The characters given are: " ++ show a ++ ", " ++ show b ++ ", and " ++ show c
```
...
```
module AnotherBigModule (module Data.Set , module AnotherBigModule) where import Data.Set import Data.Char
```
...
lower s = map toLower s which has the type
...
```
strlen [] = result where result = "No string given!" strlen f = result ++ " characters long!" where result = show (length f)
```
...
- -- Matches when the string "y" is given. agree1 "y" = "Great!"
...
```
setGreen (P col@(C { green = 0 })) = P col setGreen _ = P (C 0 0 0)
```
...
```
(reverse (dropWhile isSpace (reverse s))) in trim last ++ ", " ++ trim rst > " Haskell " ## " Curry " Curry, Haskell
```
...
```
convertOnly test change str = map (c  if test c then change c else c) str
```

Using convertOnly, we can write the l33t function which converts certain letters to numbers:

```
l33t = convertOnly isL33t toL33t where isL33t 'o' = True isL33t 'a' = True -- etc. isL33t _ = False toL33t 'o' = '0' toL33t 'a' = '4' -- etc. toL33t c = c
```
...
```
> let mult10 = multBy 10 > mult10 10 100
```
