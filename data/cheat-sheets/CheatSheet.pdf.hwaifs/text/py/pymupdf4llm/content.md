...
with ‘ -- ^ ’ for compatibility with Haddock [1], a system for documenting Haskell code.
...
_•_ "abc"  - Unicode string, sugar for
...
_•_ 1  - Integer or Floating point

_•_ 1.0, 1e10  - Floating point

_•_ 1.  - syntax error

_•_ -1  - sugar for (negate 1)

_•_ 2-1  - sugar for ((-) 2 1)
...
_•_ [1..10]  - List of numbers – 1, 2, . . ., 10 .

_•_ [100..]  - Infinite list of numbers – 100, 101,
102, . . . .

_•_ [110..100]  - Empty list; ranges only go forwards.

_•_ [0, -1 ..]  - Negative integers.

_•_ [-100..-110]  - Syntax error; need

[-100.. -110] for negatives.

_•_ [1,3..100], [-1,3..100]  - List from 1 to
...
_•_ [1,2,3]  - List of three numbers.

_•_ 1 : 2 : 3 : []  - Alternate way to write lists
using “cons” ( : ) and “nil” ( [] ).

_•_ "abc"  - List of three characters (strings are
lists).

_•_ 'a' : 'b' : 'c' : []  - List of characters
(same as "abc" ).

_•_ (1,"a")  - 2-element tuple of a number and a
string.

_•_ (head, tail, 3, 'a')  - 4-element tuple of
...
1 [http://haskell.org/haddock/](http://haskell.org/haddock/)


_⃝_ c 2009 Justin Bailey. 1 [jgbailey@codeslower.com](mailto:jgbailey@codeslower.com)
...
but can match a pattern: the shape of the value being inspected. Consider a simple data type:
...
the ‘ _ ’ token is a “wildcard” matching any value.
...
Using Maybe we can determine if any choice was
...
bottom. If anyChoice1 is reordered as follows, the
...
_⃝_ c 2009 Justin Bailey. 2 [jgbailey@codeslower.com](mailto:jgbailey@codeslower.com)
...
MyType is the type’s _name_ . MyValue1 and
MyValue are _values_ of the type and are called _con-_
_structors_ . Multiple constructors are separated with
the ‘ | ’ character. Note that type and constructor
names _must_ start with a capital letter. It is a syntax
error otherwise.
...
_type_ names, not constructors. That means this kind
...
which declares a type named User with two constructors, User and Admin . Using this type in a
...
support so-called _multi-parameter type classes_, which
...
on page 3). Continuing, we can define several instances:
...
for functions in a class. These are useful when certain functions can be defined in terms of others in
the class. A default is defined by giving a body to
one of the member functions. The canonical example is Eq, which defines /= (not equal) in terms of
== . :
...
instance declaration must always implement at
least one class member.



_⃝_ c 2009 Justin Bailey. 3 [jgbailey@codeslower.com](mailto:jgbailey@codeslower.com)
...
an argument of _any_ type, which is represented by
the type variable a above.
...
These names are referred to as _selector_ or _accessor_
...
**Deriving** Many types have common operations
which are tedious to define yet necessary, such as
the ability to convert to and from strings, compare
for equality, or order in a sequence. These capabilities are defined as typeclasses in Haskell.



Because seven of these operations are so common, Haskell provides the deriving keyword
...
It is a syntax error to specify deriving for any other
...
will be in a _monadic_ context. Statements are separated by newlines, assignment is indicated by <-,
...
A do is still required, but indention is unnecessary.
This code shows a case example, but the principle
applies to if as well:
...
That is, doesFileExist results in an IO Bool value,
...
Notice the use of return . Because do puts us “inside” the IO monad, we can’t “get out” except
...
Again, notice where return is. We don’t put it in
the let statement. Instead we use it once at the end
of the function.
...
_⃝_ c 2009 Justin Bailey. 5 [jgbailey@codeslower.com](mailto:jgbailey@codeslower.com)
...
Local functions can be defined within a function using let . The let keyword must always be followed
...
scope (including those defined by let ). In this example, mult multiplies its argument n by x, which
was passed to the original multiples . mult is used
...
**Deconstruction** The left-hand side of a let definition can also destructure its argument, in case
...
**Imports** The Haskell standard libraries are divided into a number of modules. The functionality
...
Everything means _everything_ : functions, data types
...
_⃝_ c 2009 Justin Bailey. 6 [jgbailey@codeslower.com](mailto:jgbailey@codeslower.com)


**Exports** If an export list is not provided, then all
functions, types, constructors, etc. will be available
to anyone importing the module. Note that any imported modules are _not_ exported in this case. Limiting the names exported is accomplished by adding
...
here to specify which functions, types, constructors, and classes are exported, with a few differences. If a module imports another module, it can
...
Of course, this prevents our module from patternmatching on the values of type Lexeme . We can
...
defined for a class using syntax similar to importing constructors for data types:
...
imported unless explicitly excluded. To _only_ import
...
**Exclusions** If most, but not all, names are to be
...
or other name exported by Data.Set to now be prefixed with the _alias_ (i.e., Set ) given. Here is one way
...
_⃝_ c 2009 Justin Bailey. 7 [jgbailey@codeslower.com](mailto:jgbailey@codeslower.com)
...
the above example, we can define a Phone type as
follows:
...
Phone are _not_ just String values. The typechecker
...
_•_ No runtime cost is associated with the new
type, since it does not actually produce new
values. In other words, newtypes are absolutely free!

_•_ The type-checker is able to enforce that common types such as Int or String are used in
...
their first name and the second their last. However, the order and meaning of the two arguments
...
Because type introduces a synonym, type checking
is not affected in any way. The function lower, defined as:
...
Because type is just a synonym, it cannot declare
multiple constructors the way data can. Type variables can be used, but there cannot be more than
the type variables declared with the original type.
That means a synonym like the following is possible:
...
Note that _fewer_ type variables can be used, which
...
current function. If a function is broken into multiple definitions through pattern-matching, then the
...
_⃝_ c 2009 Justin Bailey. 8 [jgbailey@codeslower.com](mailto:jgbailey@codeslower.com)


scope of the where clause extends over all guards.
In contrast, the scope of a let expression is only
the current function clause _and_ guard, if any.

## **Declarations, Etc.**


The following section details rules on function declarations, list comprehensions, and other areas of
the language.
...
bound to str is converted to lower case:
...
_n_ + _k_ **Patterns** This (sometimes controversial)
pattern-matching facility makes it easy to match
certain kinds of numeric expressions. The idea
is to define a base case (the “ _n_ ” portion) with a
constant number for matching, and then to define
other matches (the “ _k_ ” portion) as additives to the
...
bind the head of the list in l for display, while also
binding the entire list to ls in order to compute its
length:
...
expressions are tested from top to bottom. For example, neither of these functions would be very interesting:
...
_⃝_ c 2009 Justin Bailey. 9 [jgbailey@codeslower.com](mailto:jgbailey@codeslower.com)
...
the _type_ of a particular value, even if the value isn’t
present.
...
We’d rather get a Just ( _default value_ ) back instead.
Here is where a lazy pattern saves us – we can pretend that we’ve matched Just x and use that to get
...
has type Maybe a but, if not enough other information is available, Haskell must be told what a is.
...
A list comprehension consists of four types of elements: _generators_, _guards_, _local bindings_, and _targets_ .
...
and guards. Below, z is used to represent the minimum of a and b :
...
_⃝_ c 2009 Justin Bailey. 10 [jgbailey@codeslower.com](mailto:jgbailey@codeslower.com)
...
functions that take two arguments and have special syntactic support. Any so-called operator can
...
available in this form. Type signatures are a bit different, though. The operator “name” must appear
in parentheses:
...
where _precedence_ varies from 0 to 9. _Op_ can actually be any function which takes two arguments
(i.e., any binary operation). Whether the operator
is left or right associative is specified by infixl or
infixr, respectively. Such infix declarations have
no associativity.
...
_⃝_ c 2009 Justin Bailey. 11 [jgbailey@codeslower.com](mailto:jgbailey@codeslower.com)
...
“Sections” are curried operators, using parentheses. Here is add10 using sections:
...
The above is a bit verbose and can be rewriten using record syntax. This kind of “update” only sets
...
both positions. They are useful for simple functions which don’t need a name. The following determines if a string has mixed case (or is all whitespace):



_⃝_ c 2009 Justin Bailey. 12 [jgbailey@codeslower.com](mailto:jgbailey@codeslower.com)
...
_Documentation_ —Even if the compiler can figure
out the types of your functions, other programmers or even yourself might not be able
to later. Writing the type signatures on all
top-level functions is considered very good
form.


_Specialization_ —Typeclasses allow functions with
...
some cases they are needed to prevent polymorphism. A type signature is first the name of the
item which will be typed, followed by a ::, followed by the types. An example of this has already
been seen above.
...
Adrian Neumann, Barak Pearlmutter, Lanny Ripple, Markus Roberts, Holger Siegel, Leif Warner,
and Jeff Zaroyko.
...
at GitHub [2] . The latest released version of the
PDF can be downloaded from Hackage [3] . Visit
CodeSlower.com [4] for other projects and writings.



2 [http://github.com/m4dc4p/cheatsheet](http://github.com/m4dc4p/cheatsheet)
3 [http://hackage.haskell.org/cgi-bin/hackage-scripts/package/CheatSheet](http://hackage.haskell.org/cgi-bin/hackage-scripts/package/CheatSheet)
4 [http://blog.codeslower.com/](http://blog.codeslower.com/)


_⃝_ c 2009 Justin Bailey. 13 [jgbailey@codeslower.com](mailto:jgbailey@codeslower.com)
