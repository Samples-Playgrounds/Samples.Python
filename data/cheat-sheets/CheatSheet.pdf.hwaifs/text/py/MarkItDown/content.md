...
ecutable Haskell ﬁle and a printable document.
...
Comments above function deﬁnitions should
start with ‘{- |’ and those next to parameter types
with ‘-- ^’ for compatibility with Haddock1, a sys-
...
• [100..] – Inﬁnite list of numbers – 100, 101,

102, . . .

.

• [110..100] – Empty list; ranges only go for-
...
• [1,3..100], [-1,3..100] – List from 1 to
...
• 1 : 2 : 3 : [] – Alternate way to write lists
...
• (1,"a") – 2-element tuple of a number and a

string.

• (head, tail, 3, ’a’) – 4-element tuple of
two functions, a number and a character.

c(cid:13) 2009 Justin Bailey.
...
let, do and of. They cannot be used when deﬁn-
...
Unless a where clause is present. In that case, in-
...
from the ﬁrst deﬁnition in the let. If let appears
on its own line, the body of any deﬁnition must
appear in the column after the let:

Nesting & Capture Nested matching and bind-
...
deﬁnitions are given, all identiﬁers must appear in
the same column.

Keywords

Figure 1: The deﬁnition of Maybe
...
Nothing (cid:25) "No choice!"
Just (First _) (cid:25) "First!"
...
ﬁrst pattern will always succeed:

anyChoice3 ch =

case ch of

As with pattern-matching in function deﬁnitions,
...
(cid:29)avor :: a (cid:25) String

Guards
Guards, or conditional matches, can be
used in cases just like function deﬁnitions. The only
...
on page 3). Continuing, we can deﬁne several in-
stances:
...
tain functions can be deﬁned in terms of others in
the class. A default is deﬁned by giving a body to
one of the member functions. The canonical exam-
ple is Eq, which deﬁnes /= (not equal) in terms of
==. :
...
strcmp ss1 ss2
| otherwise (cid:25) False

_ (cid:25) False

Class

A Haskell function is deﬁned to work on a certain
type or set of types and cannot be deﬁned more
...
and instance declarations. A class deﬁnes one
...
We can deﬁne a class which supplies a ﬂavor for
...
MyValue1 and
MyValue are values of the type and are called con-
...
which declares a type named User with two con-
structors, User and Admin. Using this type in a
...
We can also mix type variables and speciﬁc
...
scope. Thus the “ct” preﬁx on each above. Mul-
...
mon, Haskell provides the deriving keyword
...
Two forms of deriving are possible. The ﬁrst is
...
velop. Consider the function doesFileExists from
System.Directory:
...
(cid:28)le (cid:25) do
f (cid:6) readFile (cid:28)le
putStrLn ("The (cid:28)le is " ++

show (length f)
++ " bytes long.")

doesFileExist :: FilePath (cid:25) IO Bool

The if statement has this “signature”:

if-then-else :: Bool (cid:25) a (cid:25) a (cid:25) a
...
if doesFileExist (cid:28)leName
...
(cid:21) (cid:6) doesFileExist (cid:28)leName
if (cid:21)

then return 1
else return 0

Notice the use of return. Because do puts us “in-
...
condition and get a value ﬁrst:

right2 (cid:28)leName = do
...
putStrLn "Enter a (cid:28)lename."
args (cid:6) getLine
if length args (cid:30) 0
...
Remember, if always “returns” a value.
It is an
expression, not just a control ﬂow statement. This
...
Local functions can be deﬁned within a function us-
ing let. The let keyword must always be followed
by in. The in must appear in the same column as
the let keyword. Functions deﬁned have access to
all other functions and variables within the same
scope (including those deﬁned by let). In this ex-
ample, mult multiplies its argument n by x, which
...
Deconstruction
The left-hand side of a let def-
inition can also destructure its argument, in case
sub-components are to be accessed. This deﬁnition
would extract the ﬁrst three characters from a string
...
A module can only be deﬁned in one ﬁle, though
its exports may come from multiple sources. To
make a Haskell ﬁle a module, just add a module
...
cations of the directory a particular source ﬁle is,
...
ing into your source ﬁle. To import all everything
...
port: all instance declarations in a module will be
...
We can also import types and classes deﬁned in the
module:

Qualiﬁed Imports
The names exported by a
module (i.e., functions, types, operators, etc.) can
have a preﬁx attached through qualiﬁed imports.
...
deﬁned for a class using syntax similar to import-
...
ﬁxed with the alias (i.e., Set) given. Here is one way
...
Instead,
A second form does not create an alias.
the preﬁx becomes the module name. We can write
...
speciﬁed via the hiding keyword:

import quali(cid:28)ed Char
...
useful when all local deﬁnitions and a given im-
...
Except for the preﬁx speciﬁed, qualiﬁed imports
...
creates synonyms, newtype falls somewhere be-
tween. The syntax for newtype is quite restricted—
only one constructor can be deﬁned, and that con-
structor can only take one argument. Continuing
the above example, we can deﬁne a Phone type as
follows:
...
values.
In other words, newtypes are abso-
lutely free!

• The type-checker is able to enforce that com-
mon types such as Int or String are used in
restricted ways, speciﬁed by the programmer.
...
This keyword deﬁnes a type synonym (i.e., alias).
This keyword does not deﬁne a new type, like data
...
where the ﬁrst constructor argument represents
their ﬁrst name and the second their last. How-
...
multiple constructors the way data can. Type vari-
...
Similar to let, where deﬁnes local functions and
constants. The scope of a where deﬁnition is the
current function. If a function is broken into multi-
ple deﬁnitions through pattern-matching, then the
scope of a particular where clause only applies to
that deﬁnition. For example, the function result
...
Where vs. Let A where clause can only be de-
ﬁned at the level of a function deﬁnition. Usually,
that is identical to the scope of let deﬁnition. The
...
Functions are deﬁned by declaring their name, any
...
tion can be deﬁned by “pattern-matching” on the
...
and recalling the deﬁnition of Maybe from page 2
...
“guards” in function deﬁnitions along with pattern
...
is to deﬁne a base case (the “n” portion) with a
constant number for matching, and then to deﬁne
other matches (the “k” portion) as additives to the
base case. Here is a rather inefﬁcient way of testing
...
function that determines if the ﬁrst character in a
...
the ‘_’ character is a wildcard and
...
now deﬁne a Pixel type and a function to replace
...
type. Deﬁning instances for basic types is easy:
...
might be Nothing. The following deﬁnition would
...
has type Maybe a but, if not enough other informa-
...
[c | c (cid:6) [minBound .. maxBound]

, isUpper c]

Or, to ﬁnd all occurrences of a particular break
value br in a list word (indexing from 0):

idxs word br =

[i | (i, c) (cid:6) zip [0..] word
...
There are very few predeﬁned “operators” in
Haskell—most that appear predeﬁned are actually
syntax (e.g., “=”).
Instead, operators are simply
functions that take two arguments and have spe-
cial syntactic support. Any so-called operator can
be applied as a preﬁx function using parentheses:

3 + 4 (cid:30) (+) 3 4

To deﬁne a new operator, simply deﬁne it as a nor-
...
Allowable symbols which can be used to deﬁne op-
erators are:

# $ % & * + (cid:0) / < = > ? @ (cid:8) ^ | - ~

However, there are several “operators” which can-
not be redeﬁned. They are: <-, -> and =. The last,
=, cannot be redeﬁned by itself, but can be used as
part of multi-character operator. The “bind” func-
tion, >>=, is one example.

Precedence & Associativity
The precedence
and associativity, collectively called ﬁxity, of any
operator can be set through the infix, infixr and
infixl keywords. These can be applied both to
top-level functions and to local deﬁnitions. The
syntax is:
...
is left or right associative is speciﬁed by infixl or
...
in(cid:28)xl 8 ‘plus1‘
plus1 a b = a + b
in(cid:28)xl 7 ‘mult1‘
mult1 a b = a * b
...
Redeﬁning division as right associative:
...
Notice that l33t has no arguments speciﬁed. Also,
the ﬁnal argument to convertOnly is not given.
However, the type signature of l33t tells the whole
story:

l33t :: String (cid:25) String
...
ﬁrst is the conversion function which converts indi-
...
with new values in the ﬁelds that “changed.” For
example, using the Color type deﬁned earlier, we
can write a function that sets the green ﬁeld to zero
easily:
...
values for the ﬁeld(s) speciﬁed and copies the rest:
...
matching to set the green and blue ﬁelds to equal
the red ﬁeld:
...
syntax (“C { red = r}”) to get the inner red ﬁeld.
...
They can be deﬁned at any time like so:

(cid:8)c (cid:25) (c, c)

which deﬁnes a function which takes an argument
...
convertUpper :: (Char (cid:25) Char)

(cid:25) String (cid:25) String
...
However, for efﬁciency or other reasons you
...
negateAll :: [Int] (cid:25) [Int]

canParseInt x = show ((read x) :: Int)

mixedCase str =

all ((cid:8)c (cid:25) isSpace c (cid:31)
isLower c (cid:31)
isUpper c) str
...
Documentation—Even if the compiler can ﬁgure
...
stration of this is the so-called “show . read” prob-
lem:

negateAll :: Num a => [a] (cid:25) [a]
...
at GitHub2. The latest released version of the
PDF can be downloaded from Hackage3. Visit
CodeSlower.com4 for other projects and writings.
