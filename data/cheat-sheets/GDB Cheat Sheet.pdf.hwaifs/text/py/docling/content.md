...
function\_name Break/watch the named function.
...
break/watch &lt;where&gt; if &lt;condition&gt; Break/watch at the given location if the condition is met. Conditions may be almost any C ex- pression that evaluate to true or false.
...
Show call stack, also print the local va- backtrace full where full riables in each frame.
...
Go to next instruction (source line) but don ʻ t dive into functions.
...
/format &lt;what&gt; Like 'print', but print the information after each stepping instruction.

Remove the 'display' with the given undisplay &lt;display#&gt; number.

En- or disable the 'display' with the gi- enable display &lt;display#&gt; disable display &lt;display#&gt; ven number.
...
handle &lt;signal&gt; &lt;options&gt; Set how to handle signles. Options are: (no)print : (Don ʻ t) print a message when signals occurs. (no)stop : (Don ʻ t) stop the program when signals occurs. t) pass the signal to the

(no)pass : (Don ʻ program.
...
info breakpoints Print informations about the break- and
...
Print all directories in which GDB sear- ches for source files.

show listsize Print how many are shown in the 'list'

command.

whatis

variable\_name Print type of named variable.