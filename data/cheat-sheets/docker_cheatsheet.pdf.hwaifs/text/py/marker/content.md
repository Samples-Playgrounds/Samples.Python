...
Build an Image from a Dockerfile Build an Image from a Dockerfile without the cache docker build -t <image\_name> . –no-cache List local images docker images Delete an Image docker rmi <image\_name> Remove all unused images docker image prune docker build -t <image\_name> .
...
Run a container with and publish a container's port(s) to the host. docker run -p <host\_port>:<container\_port> <image\_name>

Run a container in the background docker run -d <image\_name>
