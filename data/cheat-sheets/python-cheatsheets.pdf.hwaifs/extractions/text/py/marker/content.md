...
| 25<br>>>> x%2       | Remainder of a variable         |
| 1<br>>>> x/float(2) | Division of a variable          |
| 2.5                 |                                 |
...
>>> import numpy as np Selective import >>> from math import pi
...
Download serialized state of all widget models in use ..... Save notebook with Widgets ..... with interactive widgets ..... widgets ..... widgets ..... widgets ..... widgets ..... widgets ..... widgets ..... widgets ..... widgets ..... widgets ..... widgets ..... Embed current
...
| >>> a.s | shape       | Array dimensions                     |
|---------|-------------|--------------------------------------|
| >>> ler | n (a)       | Length of array                      |
| >>> b.r | ndim        | Number of array dimensions           |
| >>> e.s | size        | Number of array elements             |
| >>> b.c | ltype       | Data type of array elements          |
| >>> b.c | ltype.name  | Name of data type                    |
| >>> b.a | astype(int) | Convert an array to a different type |
...
| array([[-0.5, 0. , 0.],                                  |                                |
| [-3., -3., -3.]])                                        | Subtraction                    |
| >>> np.subtract(a,b) >>> b + a                           | Addition                       |
| array([[ 2.5, 4., 6.],                                   | Addition                       |
| [5., 7., 9.]])                                           |                                |
| >>> np.add(b,a)                                          | Addition                       |
| >>> a / b                                                | Division                       |
| array([[ 0.66666667, 1. , 1. ]]<br>[ 0.25 , 0.4 , 0.5 ]] |                                |
|                                                          |                                |
| >>> np.divide(a,b)<br>>>> a * b                          | Division                       |
| >>> a ^ D<br>array([[ 1.5, 4., 9.],                      | Multiplication                 |
| [ 4. , 10. , 18. ]])                                     |                                |
...
|   | >>> a.sort()<br>>>> c.sort(axis=0) | Sort an array<br>Sort the elements of an array's axis |
|---|------------------------------------|-------------------------------------------------------|
| ш | >>> c.sort(axis=0)                 | Sort the elements of an array's a                     |
...
| Transposing Array |  |                                        |
|-------------------|--|----------------------------------------|
|                   |  | Permute array dim<br>Permute array dim |

1 2 3

#### **Changing Array Shape** >>> b.ravel() >>> g.reshape(3,-2)
...
>>> np.insert(a, 1, 5) >>> np.delete(a,[1]) **Combining Arrays** 
...
| >>> np.mgrid[0:5,0:5]<br>>>> np.ogrid[0:2,0:2]<br>>>> np.r_[[3,[0]*5,-1:1:10] |                                   |
|-------------------------------------------------------------------------------|-----------------------------------|
| >>> np.c_[b,c]                                                                | Create stacked column-wise arrays |

#### Shape Manipulation

| >>> np.transpose(b) >>> b.flatten() | Permute array dimensions<br>Flatten the array |
|-------------------------------------|-----------------------------------------------|
| >>> np.hstack((b,c))                | Stack arrays horizontally (column-wise)       |
| >>> np.vstack((a,b))                | Stack arrays vertically (row-wise)            |
| >>> np.hsplit(c,2)                  | Split the array horizontally at the 2nd inde  |
| >>> np.vpslit(d.2)                  | Split the array vertically at the 2nd index   |

#### **Polynomials**

| p = poly1d([3,4,5]) | Create a polynomial objec |
|---------------------|---------------------------|
|                     |                           |
...
| >>><br>>>> | np.imag(c)<br>np.real_if_close(c,tol=1000) | Return the real part of the array elements Return the imaginary part of the array element Return a real array if complex parts close to 0 |
|------------|--------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------|
...
>>> linalg.det(A) Solving linear problems
...
Inverse Tranpose matrix Conjugate transposition
...
| ı | >>> C[C > 0.5] = 0             |                                 |
| ı | >>> H = sparse.csr matrix(C)   | Compressed Sparse Row matrix    |
...
| ١ | >>> sparse.isspmatrix csc(A)   | Identify sparse matrix          |
...
Set index a of Series  ${\tt s}$  to 6
...
>>> pd.read\_sql("SELECT \* FROM my\_table;", engine)

>>> pd.read\_sql\_table('my\_table', engine)
>>> pd.read\_sql\_guery("SELECT \* FROM\_my\_table;", engine)

 ${\tt read\_sql}$  () is a convenience wrapper around  ${\tt read\_sql\_table}$  () and  ${\tt read\_sql}$  query()

>>> pd.to\_sql('myDf', engine)
...
| >>> df.min()/df.max() >>> df.idxmin()/df.idxmax() >>> df.describe() >>> df.mean() >>> df.median()  Minimum/maximum values Minimum/Maximum index valu Summary statistics Mean of values Median of values | >>> df.cumsum() >>> df.min()/df.max() >>> df.idxmin()/df.idxmax() >>> df.describe() >>> df.mean() | Summary statistics<br>Mean of values |
|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------|--------------------------------------|
|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------|--------------------------------------|
...
- >>> lr.fit(X, y) >>> knn.fit(X train, v train)
- >>> svc.fit(X train, v train)
...
- param grid=params) >>> grid.fit(X train, y train)
...
- >>> room skreain.grid search import kandomizedsea
  ...
  - param distributions=params, n iter=8.
...
>>> ax legend(loc='best') >>> ax.xaxis.set(ticks=range(1,5),

xlabel='X-Axis')

ticklabels=[3.100,-12."foo"])>>> ax.tick params(axis='v', direction='inout'. length=10)
...
|  | >>> | <pre>ax1.spines['top'].set visible(False)</pre> |
|--|-----|-------------------------------------------------|
|  | >>> | ax1.spines['bottom'].set position(('outward     |

Make the top axis line for a plot invisible (10) Move the bottom axis line outward
...
| >>> axes[1,1].quiver(y,z) Pl | Add an arrow to the axe<br>Plot a 2D field of arrows<br>Plot a 2D field of arrows |
|------------------------------|-----------------------------------------------------------------------------------|
|------------------------------|-----------------------------------------------------------------------------------|
...
#### 2D Data or Images >>> fig. ax = plt subplots()
...
**DataCamp** Learn Python for Data Science Inte

![](_page_6_Picture_67.jpeg)
...
>>> f. ax = plt supplots (figsize= (5.6)) Create a figure and one supplot
...
#### linestvles=["-","--"]) >>> sns.boxplot(x="alive",

markers=["^"."o"].

v="age". hue="adult male". data=titanic) >>> sns.boxplot(data=iris,orient="h")

>>> sns.violinplot(x="age", v="sex". hue="survived", data=titanic)
...
| Context Functions                                                                        |                                                                                                          |
|------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------|
| <pre>&gt;&gt;&gt; sns.set_context("talk") &gt;&gt;&gt; sns.set_context("notebook",</pre> | Set context to "talk"<br>Set context to "notebook",<br>scale font elements and<br>override param mapping |

#### (Re)set the seaborn default Set the matplotlib parameters
...
```
>>> from bokeh.io import output_file, show >>> output file('my bar chart.html', mode='cdn')
```
