...
[this post were gathered with a recent daily build](https://github.com/dotnet/installer/blob/main/README.md#installers-and-binaries) - f .NET 7 RC1.
...
- f the code on the startup path (e.g. all of the code that needs to be run before a web server can
...
- nly a handful of times, and it would take more time to optimize them than it does to just execute
...
- riginally envisioned as a way to optimize startup while not hurting throughput, it’s become much
...
- nly have been initialized by the time the tier-1 code executes but their values won’t ever change. And
...
- f OSR comes into play when combined with another exciting feature: dynamic PGO.
...
- f a single comparison and branch.
...
- f this is obvious if we run this as a benchmark:
...
- nly pay for the bounds checks in the slow path. You can see this in the generated assembly (if you
...
- perations out of the loop, thereby avoiding the overhead of recomputing the value per iteration.
...
- bject, and the length of an array is stored in the object just after that method table pointer (which is
...
- f the array and comparing the length to 0; that makes sense, since the length can’t be negative, and
...
- ptimizations into the JIT. The `Regex` compiler and source generator handle some cases of regular
...
- nly ones. There’s also the need to use the cheapest instructions possible. In .NET 6, with a method
...
- f moving it, it’s sign-extending it; that’s the “sxd” part of the instruction name (sign-extending means
...
- f an array and of a span is non-negative, and since we just bounds checked `i` against the `Length`, we
...
- f array access overhead that’s been there forever, but that’s so well known there are even old FxCop
...
- f the world here by doing an expansion of a multidimensional array access early in the JIT’s pipeline,
...
and the thousands out three levels. Here’s what `[DisassemblyDiagnoser]` - utputs for .NET 6:
...
- nly five instructions: increment `ebp` (which at this point is the `ones` counter value), and if it’s still less
...
performing the `thousands * 1000` and `hundreds * 100` - perations, highlighting that these

- perations which could have been hoisted out further were left stuck in the next-to-innermost loop.
...
- f the loops, and we can see the outermost loop has the `imul ebx,esi,3E8` (for the thousands
...
- nly constants at compile-time rather than generating the code to compute the value at run-time.
...
- f `A` to `B`, such that the JIT effectively sees the body of `B` as the equivalent of `return 23 * 2;` . At that
...
If we look at the assembly code generated for `Compute1` - n .NET 6, it looks like what we’d hope for.
...
- f a particular value? You could walk the list comparing one element at a time, and that would be fine
...
should be used for `Vector<T>` - perations. You already saw this earlier when discussing the output of
...
- f which are .NET APIs that map down to one of these SIMD instructions. These intrinsics enable an
...
developer who wants or needs to go beyond what the high-level `Vector<T>` - ffers can choose to
...
- rder to potentially double throughput further on platforms that have 256-bit width vectors. Think of
...
- ur cake and eat it, too? Let’s also add a `Vector256<T>` path. To do that, we literally copy/paste our
...
- n really hot paths, that’s often on the smaller side of the wins inlining brings. The bigger wins are
...
- thers that are specific to Arm, but there are plenty more. To rattle off some of them:
...
pattern `(a * b) + c` with integers and fold that into a single `madd`    - r `msub` instruction, while
...
- ne involves computing a hash code and using a mod operation to map to the right bucket. Other
...
- f `ngen`, and .NET Core has it in the form of `crossgen` . Both of those solutions involve a standard .NET
...
- n reflection emit for good performance will need another scheme. We can see that, for example, with
...
time, with no support for `Assembly.LoadFrom` - r the like. That means at compile time, the compiler
...
- ptimization of that IR, and then once a certain threshold of call counts has been hit, then take the
...
[dotnet/runtime#64867. Previously, any methods with](https://github.com/dotnet/runtime/pull/64867) `catch` - r `filter` exception handling clauses
...
- ccurs, switching over to the interpreter for the remainder of that method call’s execution. Since many
...
knows how to translate `Vector128<T>` - perations to WASM’s SIMD instruction set, such that code
...
represent its types), and `Enum` - ut of native into managed.
...
underlying support for `AssemblyName` - ut of native runtime code into managed code in CoreLib. That
in turn has an impact on anything that uses it, such as when using `Activator.CreateInstance`

- verloads that take assembly names that need to be parsed.
...
|Method|Runtime|Mean|Ratio|Allocated|AllocRatio|
|---|---|---|---|---|---|
...
|Method|Runtime|Mean|Ratio|CodeSize|
|---|---|---|---|---|
...
- ther system. And the code would be inspectable, viewable by users to understand exactly what work
...
- pt-in to having it persisted on disk by adding a
...
- In .NET 6, there were almost 3000 `[DllImport]` uses throughout the core .NET libraries. As of my
...
[To highlight just a few more, dotnet/runtime#63320](https://github.com/dotnet/runtime/pull/63320) introduces a new
...
[like dotnet/runtime#67635 and dotnet/runtime#68173](https://github.com/dotnet/runtime/pull/67635) added new marshaling types that encompass
...
[common marshalling needs). Really usefully, dotnet/runtime#71989](https://github.com/dotnet/runtime/pull/71989) added support for marshaling
...
[And dotnet/runtime#69043 consolidated logic to be shared between the runtime’s marshalling](https://github.com/dotnet/runtime/pull/69043)
...
method and a finalizer for closing it. That way, if you neglect to `Dispose` - f the `SafeHandle` in order to
...
- bject. To deal with that, this code would have been more robustly written as:
...
That’s because calling `Dispose` - n a `SafeHandle` may end up invoking the `SafeHandle` ’s
...
`SetLastError=true` - n it, it can overwrite the very error code for which we’re about to throw. Hence,
...
[dotnet/runtime#72222,](https://github.com/dotnet/runtime/pull/72222) [dotnet/runtime#72203, and dotnet/runtime#72279 all found and fixed many](https://github.com/dotnet/runtime/pull/72203)

- ccurrences of `SafeHandle` s being left for finalization (many thanks to the diagnostics put in place in
...
- n the global queue. This PR addresses this for such larger machines by introducing additional global
...
- ne additional thread is requested at a time, and after that thread has dequeued its first work item, it
...
- ne of those configurations). At first glance you might think, “hey, this is a performance test about
...
most part, this has worked well, with the majority of `ExecutionContext` s being able to represent
...
- ptimize from one to four of them rather than one to three. While this improves throughput a bit, its
main intent was to improve allocation, which is does over .NET 6 by ~20%.

|Method|Runtime|Mean|Ratio|CodeSize|Allocated|AllocRatio|
|---|---|---|---|---|---|---|
...
`float.TryParse` [, etc.). dotnet/runtime#62301 from [@CarlVerret](https://github.com/CarlVerret)](https://github.com/dotnet/runtime/pull/62301)
...
`bool.TryParse` and `bool.TryFormat` [were also improved. dotnet/runtime#64782](https://github.com/dotnet/runtime/pull/64782) streamlined these
implementations by using `BinaryPrimitives` to perform fewer writes and reads. For example, instead

- f `TryFormat` writing out “True” by doing:
...
search via the internal `SpanHelpers.IndexOf` (the worker routine behind `IndexOf` - n spans, strings,
...
[would box the arguments. dotnet/runtime#68077](https://github.com/dotnet/runtime/pull/68077) fixes this by ensuring nullable enums get mapped
...
Not to be left out, `Guid` [’s equality operations also get faster, thanks to dotnet/runtime#66889 from](https://github.com/dotnet/runtime/pull/66889)
...
`DateTime` [equality is also improved. dotnet/runtime#59857](https://github.com/dotnet/runtime/pull/59857) shaves some overhead off of

`DateTime.Equals` . `DateTime` is implemented with a single `ulong _dateData` field, where the majority

- f the bits store a ticks offset from 1/1/0001 12:00am and where each tick is 100 nanoseconds, and
...
[“Euclidean Affine Functions and Applications to Calendar Algorithms”](https://arxiv.org/pdf/2102.06959.pdf) in order to improve
...
[shipped publicly). dotnet/runtime#67714 added support for user-defined](https://github.com/dotnet/runtime/pull/67714) `checked` - perators, a new
C# 11 feature that enables both `unchecked` and `checked` variations of operators to be exposed, with
the compiler picking the right one based on the `checked` [context. dotnet/runtime#68096 also added](https://github.com/dotnet/runtime/pull/68096)
support for the new C# 11 unsigned right shift operator ( `>>>` [). And dotnet/runtime#69651,](https://github.com/dotnet/runtime/pull/69651)
...
[performance in PRs like dotnet/runtime#68226 and dotnet/runtime#68183, which use the interfaces](https://github.com/dotnet/runtime/pull/68226)

- verloads of these methods for `int`, `long`, `float`, `double`, and `decimal` . The GitHub summary of the
...
separate `ParseOctalAsUInt32` and `ParseOctalAsUInt64` [methods, dotnet/runtime#74281]](https://github.com/dotnet/runtime/pull/74281)
...
[C# 11 includes the aforementioned support for user-defined checked operators, enabling the generic](https://github.com/dotnet/csharplang/blob/main/proposals/checked-user-defined-operators.md)
...
[dotnet/runtime#65584](https://github.com/dotnet/runtime/pull/65584) did the same for `Max` and `Min` so that the Arm-specific `fmax` and `fmin`
...
[dotnet/runtime#55121](https://github.com/dotnet/runtime/pull/55121) from [@key-moon](https://github.com/key-moon) also improves parsing, but
...
Also related to `BigInteger` [(and not just for really big ones), dotnet/runtime#35565 from](https://github.com/dotnet/runtime/pull/35565)
...
- r that are ASCII letters:


- r other such groups. Interestingly, there’s wide-spread variation in how such checks are coded, often
...
`MemoryExtensions.CommonPrefixLength` [method, introduced by dotnet/runtime#67929. This accepts](https://github.com/dotnet/runtime/pull/67929)
...
`value0, T value1, ...)` searches for the first occurrence of something that’s _not_ equal to `value0`,
...
[vectorization scheme was overhauled for significant gain. Let’s start with dotnet/runtime#63285,](https://github.com/dotnet/runtime/pull/63285)
which yields huge improvements for many uses of `IndexOf` and `LastIndexOf` for “substrings” of `byte` s
and `char` s. Previously, given a call like `str.IndexOf("hello")`, the implementation would essentially
...
- ut of the vectorized loop in order to do the full string comparison. Instead, the PR implements an
...
[Another example of significantly changing the algorithm employed is dotnet/runtime#67758, which](https://github.com/dotnet/runtime/issues/67758)
...
compare the target string, except performing a `ToUpper` - n every character in order to do it in a caseinsensitive manner. Now with this PR, which is based on approaches previously used by `Regex`, if the
...
as we’re now doing a vectorized `IndexOfAny('S', 's')` - r `IndexOfAny('E', 'e')` rather than
[manually walking each character and comparing it. (dotnet/runtime#73533 uses the same approach](https://github.com/dotnet/runtime/pull/73533)

[Another example comes from dotnet/runtime#67492](https://github.com/dotnet/runtime/pull/67492) from [@gfoidl](https://github.com/gfoidl). It
...
[dotnet/runtime#60974](https://github.com/dotnet/runtime/pull/60974) from [@alexcovington](https://github.com/alexcovington) broadens the
...
checking whether an element was found, code can use either `>= 0` - r `!= -1`, and when checking

whether an element wasn’t found, code can use either `< 0` - r `== -1` . It turns out that the code
...
[dotnet/runtime#60463](https://github.com/dotnet/runtime/pull/60463) from [@nietras](https://github.com/nietras) used `IndexOfAny` in
...
- ptimizing these methods. In previous releases, some of that energy has been in the form of using
...
[One of my favorite changes in the whole release is dotnet/runtime#65288 and is squarely in this area.](https://github.com/dotnet/runtime/pull/65288)
...
- ne char at a time, e.g. doing a single read of four chars as a `long` and a single comparison of that

`long` against the expected combination of those four chars. The result is beautiful. Making it even
[better is dotnet/runtime#66095, which adds to this support for](https://github.com/dotnet/runtime/pull/66095) `OrdinalIgnoreCase` . Remember those
...
- rdering issues, such that the source generator would need to emit code that could handle running

- n either little-endian or big-endian machines. The JIT has no such problem, as it’s generating the
...
much easier to read code utilizing `StartsWith` [that’s just as fast (dotnet/runtime#65222 and](https://github.com/dotnet/runtime/pull/65222)
[dotnet/runtime#66339). Wins all around. (This could only be removed from](https://github.com/dotnet/runtime/issues/66339) `RegexOptions.Compiled`
[after dotnet/runtime#68055, which fixed the ability for the JIT to recognize these string literals in](https://github.com/dotnet/runtime/pull/68055)

`DynamicMethod` s, which `RegexOptions.Compiled` uses with reflection emit to spit out the IL for the
...
- ften refer to `_firstChar` directly when the first character needs to be consulted, as it’s typically faster
...
`StartsWith(char value)` - n `string` . In .NET 6, the implementation was:
...
(Another example of `IsKnownConstant` [being used comes from dotnet/runtime#64016, which uses it](https://github.com/dotnet/runtime/pull/64016)
...
[Finally, dotnet/runtime#67202 and dotnet/runtime#73475 employ](https://github.com/dotnet/runtime/pull/67202) `Vector128<T>` and `Vector256<T>`
...
converting from UTF8 bytes to and from `string` - r formatting and parsing hex values. Such
...
- f representing arbitrary binary data (think `byte[]` ) across mediums that only support text, encoding
...
[7, where dotnet/runtime#71795 and dotnet/runtime#73320](https://github.com/dotnet/runtime/pull/71795) vectorize the `ToBase64String`,
...
`Encoding.UTF8.GetByteCount` [. (And in the same vein, dotnet/runtime#67192](https://github.com/dotnet/runtime/pull/67192) changed the internal
...
`Encoding.UTF8` [was also improved a bit. In particular, dotnet/runtime#69910](https://github.com/dotnet/runtime/pull/69910) streamlined the
...
are in this case. For several releases now, when the C# compiler sees a `byte[]` (or `sbyte[]` - r `bool[]` )
...
|Method|Mean|Ratio|Allocated|AllocRatio|
|---|---|---|---|---|
...
to `AppendFormat` [. One interesting change is dotnet/runtime#64405, which achieved two related](https://github.com/dotnet/runtime/pull/64405)
...
- bjects around on the heap, which it might do in order to compact the heap (to, for example, avoid
...
- ther object that was shifted to this location. There are two ways for dealing with this. The first is a
...
- n GCs themselves). There are also various optimizations inhibited by pinning. With all of the advents
...
`StringBuilder` [itself was then further updated by dotnet/runtime#64922](https://github.com/dotnet/runtime/pull/64922) from [@teotsirpanis](https://github.com/teo-tsirpanis), which improves the `Insert` methods. It used to be that
the `Append(primitive)` methods on `StringBuilder` (e.g. `Append(int)` ) would call `ToString` - n the
...
`ArrayPool<char>` [arrays. And with dotnet/runtime#64522](https://github.com/dotnet/runtime/pull/64522) [and dotnet/runtime#69683, many of the](https://github.com/dotnet/runtime/pull/69683)
...
for `ref` fields. What is a `ref` field? You’re familiar with `ref` s in C# in general, and we’ve already
...
- f C# 11 and .NET 7, `ref structs` can now contain `ref` fields, which means `Span<T>` today is literally
...
- f many PRs first into a feature branch. `ref` fields alone doesn’t itself automatically improve
...
- f the method, which is bad. Hence how this relates to `ref` fields: because `ref` fields are now a thing,
the compiler’s rules for safe-handling of `ref` s have been updated, which in turn enables us to expose
...
instance to store the `ref` (note that this was already the case with `ref struct` s passed to methods on
...
to do. Enter `scoped` . The new C# keyword does exactly what we just wished for: put it on a `ref` - r `ref`
...
method. For a call site, the compiler sees the `[UnscopedRef]` - n the member being invoked, and then
...
[Another impactful span-related change comes in dotnet/runtime#70095](https://github.com/dotnet/runtime/pull/70095) from [@teotsirpanis](https://github.com/teo-tsirpanis). `System.HashCode` ’s goal is to provide a fast, easy-to-use
...
then the `int` [rather than the other way around. So for example when dotnet/runtime#71274 from](https://github.com/dotnet/runtime/pull/71274)

[@huoyaoyuan](https://github.com/huoyaoyuan) changed `BigInteger.GetHashCode` to use

`HashCode.AddBytes`, it coded the method to first call `AddBytes` with the `BigInteger` ’s `_bits` and _then_
...
- f this in several additional places.
...
- f two chars, the implementation would extract the two chars from the array and pass them off to the
same two-argument implementation). Multiple other PRs similarly removed array allocations.
[dotnet/runtime#60409](https://github.com/dotnet/runtime/pull/60409) removed a single-char array that was cached to be able to pass it to

`string.Split` and replaced it with usage of the `Split` - verload that directly accepts a single `char` .
...
[Back in May, I shared a fairly detailed post about the improvements coming to Regular Expressions in](https://devblogs.microsoft.com/dotnet/regular-expression-improvements-in-dotnet-7)
...
processing of `Regex` - ver to using a new engine based in finite automata. It has two primary modes of
...
- f work per character in the input. In the case of a DFA, that constant is very small. With an NFA, that
...
- riginal pattern against a next `t` . What if it was against an `h` instead? Following the same logic as for
...
- r `h` ), and so we just end up with that same subexpression. But against the right side of the
...
- ut of every node (e.g. every node could need a ~65K element table); that would significantly increase
...
- nly needs to determine whether there is a match and doesn’t need to compute any additional
...
- bjects to make the handling of the lists of states ammortized allocation-free.
...
- f state rather than the implicit one. The nice thing about this is the amount of state you can store is
...
[dotnet/runtime#60786](https://github.com/dotnet/runtime/pull/60786) from [@olsaarik](https://github.com/olsaarik) specifically in the
...
- n the stack, it forks off continued execution into another thread. Hitting this condition is expensive,
...
- f the new APIs likely also misrepresents how much work was necessary to enable them, in particular
because the new APIs all support `ReadOnlySpan<char>` inputs into the regex engines.

[dotnet/runtime#65473](https://github.com/dotnet/runtime/pull/65473) brings `Regex` into the span-based era of .NET, overcoming a significant
...
returned to callers. `Regex.Match` returns a `Match` - bject that represents the first match in the input,
and that `Match` - bject exposes a `NextMatch` method that enables moving to the next match. That
means the `Match` - bject needs to store a reference to the input, so that it can be fed back into the
...
- bject, since `ref structs` can only live on the stack and not the heap. That alone would make it a
challenge to support spans, but the problem is even more deeply rooted. All of the regex engines rely

- n a `RegexRunner`, a base class that stores on it all of the state necessary to feed into the
...
- ptimization to skip past input positions that couldn’t possibly start a match and then `Go` performing
...
[dotnet/runtime#66129,](https://github.com/dotnet/runtime/pull/66129) [dotnet/runtime#66178, and dotnet/runtime#72728, all of which clean up](https://github.com/dotnet/runtime/pull/66178)
...
how many matches there are in the input, can bypass using `Match` - r `MatchCollection`, and thus can
[easily operate over spans as well; dotnet/runtime#64289](https://github.com/dotnet/runtime/pull/64289) added `string` - based overloads, and
[dotnet/runtime#66026](https://github.com/dotnet/runtime/pull/66026) added span-based overloads. We can optimize `Count` further by passing
...
- f the input. And if it then also needs to compute capture information, that requires yet another
...
[dotnet/runtime#68242](https://github.com/dotnet/runtime/pull/68242) updates the engines to receive this additional information, such that methods


So, `IsMatch` and `Count` can work with spans. But we still don’t have a method that lets you actually get

[dotnet/runtime#67794.](https://github.com/dotnet/runtime/pull/67794) `EnumerateMatches` is very similar to `Match`, except instead of handing back a
...
- ne character. Some of these were more optimized than others, e.g. a left-to-right, case-sensitive,
...
- verhead at match time) to add additional strategies.

[dotnet/runtime#60888](https://github.com/dotnet/runtime/pull/60888) then added the first additional strategy. The implementation was already
...
[regex benchmark. One of those is for URIs:](https://github.com/mariomka/regex-benchmark/blob/17d073ec864931546e2694783f6231e4696a9ed4/csharp/Benchmark.cs#L23)
...
- f vectorizing a search for such a character class. However, this pattern is interesting in that it begins
...
- n, until we’ve consumed all the `'a'` s such that the rest of the pattern has a chance of matching the
...
- pportunities to transform portions of the tree into a behaviorally equivalent alternative that will be
...
- ften arise as a result of other transformations rather than being something a developer typically
writes by hand, just as there are optimizations in the JIT where you might look at them and say “why

- n earth is that something a developer would write” but it ends up being a valuable optimization
...
- ptimizations. First, it enables rewriting alternations into alternations of alternations, e.g. transforming
...
[avoiding the resulting complications, dotnet/runtime#66432](https://github.com/dotnet/runtime/pull/66432) [and then dotnet/runtime#71765 teaches](https://github.com/dotnet/runtime/pull/71765)
the source generator to use the new `file-local types` [features in C# 11 (dotnet/roslyn#62375).](https://github.com/dotnet/roslyn/pull/62375)
...
- ne of the more time-consuming aspects of matching; if you imagine having to evaluate this logic for
...
- ptimizations around the handling of “match anything” can kick in.
...
[Or dotnet/runtime#68924, which taught the source generator how to use all of the new](https://github.com/dotnet/runtime/pull/68924) `char` ASCII
...
[[@Clockwork-Muse](https://github.com/Clockwork-Muse), and dotnet/runtime#62325 from](https://github.com/dotnet/runtime/pull/62325)

[@Clockwork-Muse](https://github.com/Clockwork-Muse) all converted `Regex` usage over to using

- verhead due to not needing to construct a `Match` instance and due to being able to avoid more
...
needing `Match` - bject allocations. The PR also entirely removed at least one regex usage that was just
...
`RegexOptions.Compiled` - r `RegexOptions.NonBacktracking` is used; instead, _any_ use of an overload
that takes a `RegexOptions` will result in that code continuing to be referenced. By getting rid of the

- ptions, we increase the chances that no code in the app is using this constructor, which would in turn
...
[dotnet/runtime#70850](https://github.com/dotnet/runtime/pull/70850) from [@grbell-ms](https://github.com/grbell-ms), the builder’s `Sort` method is
...
[dotnet/runtime#61196](https://github.com/dotnet/runtime/pull/61196) from [@lateapexearlyspeed](https://github.com/lateapexearlyspeed) brings
...
[red/black tree](https://en.wikipedia.org/wiki/Red%E2%80%93black_tree) as its internal data structure, and it uses a `Log2` - peration to determine the maximum
...
- bjects. Essentially, it’s a thread-safe dictionary that doesn’t maintain strong references to anything it
...
[dotnet/runtime#64470](https://github.com/dotnet/runtime/pull/64470) is the result of analyzing various real-world code bases for use of

`Enumerable.Min` and `Enumerable.Max`, and seeing that it’s very common to use these with arrays,

- ften ones that are quite large. This PR updates the `Min<T>(IEnumerable<T>)` and

`Max<T>(IEnumerable<T>)` - verloads when the input is an `int[]` - r `long[]` to vectorize the
...
|Method|Runtime|Length|Mean|Ratio|Allocated|AllocRatio|
|---|---|---|---|---|---|---|
...
- f the `int[]` actually being a `uint[]` (or e.g. `DayOfWeek[]` ), and minimize the performance penalty of
...
actually either a `T[]` - r a `List<T>` ; as with the previous PR, it uses the `GetType() == typeof(T[])`
...
[then improved in dotnet/runtime#71564. One of the most popular methods in LINQ is](https://github.com/dotnet/runtime/pull/71564)
...
|Method|Length|Mean|Ratio|Allocated|AllocRatio|
|---|---|---|---|---|---|
...
- peration to complete are able to be unblocked. There have been several improvements of this ilk in

.NET 7.


In some cases, it comes from adding cancelable overloads where things weren’t previously cancelable
[at all. That’s the case for dotnet/runtime#61898 from [@bgrainger](https://github.com/bgrainger),](https://github.com/dotnet/runtime/pull/61898)
...
`File.ReadLinesAsync` method. This produces an `IAsyncEnumerable<string>` - f the lines in the file,
is based on a simple loop around the new `StreamReader.ReadLineAsync` - verload, and is thus itself
fully cancelable.
...
- verride `ReadAsync`, attempts to cancel a call to its `ReadAsync` will be minimally effective. It does an
...
- r overlapped I/O based on the `PipeOptions.Asynchronous` - ption specified at construction. And, on
...
- .NET Framework 4.8: No cancellation support. The pipe `Stream` - derived types didn’t even
   ...
   - verride `ReadAsync`    - r `WriteAsync`, so all they got was the default up-front check for
...
changed to issue zero-byte reads, and would `await` a `Task.WhenAny`   - f both that operation’s
...
to an anonymous pipe. This in tern enabled every `PipeStream`   - n Unix to be implemented in
...
- verlapped I/O for anonymous pipes today, so for anonymous pipes and for named pipes opened for
...
|Method|Runtime|Cancelable|Named|Mean|Ratio|Allocated|AllocRatio|
|---|---|---|---|---|---|---|---|
...
- nly performs that `stat` if opening the file fails. If opening the file succeeds, the code was already
...
bytes. But the biggest benefit of this PR is that it takes advantage of `IOCTL-FICLONERANGE` - n Linux.
...
[same and the file system supports the operation. In a similar vein, dotnet/runtime#64264 from](https://github.com/dotnet/runtime/pull/64264)

[@tmds](https://github.com/tmds) further improves `File.Copy` / `FileInfo.CopyTo` by utilizing

`copy_file_range` - n Linux if it’s supported (and only if it’s a new enough kernel that it addresses
...
- n Unix. The implementation of these methods opens a `FileStream` - r a `SafeFileHandle` directly,
...
implementation of `FileOptions.SequentialScan` - n Unix requires an additional syscall via
...
[do that,” and so dotnet/runtime#59247 from [@tmds](https://github.com/tmds) simply stops passing](https://github.com/dotnet/runtime/pull/59247)
...
- r as a file, but a single `stat` call suffices for both.
...
[Syscalls were also reduced as part of support for memory-mapped files. dotnet/runtime#63754 takes](https://github.com/dotnet/runtime/pull/63754)
...
than `Open` - r `CreateNew`, which means we can trivially avoid the extra system call in the majority case.
[dotnet/runtime#63790](https://github.com/dotnet/runtime/pull/63790) also helps here, in two ways. First, throughout the `CreateFromFile` - peration,
...
[Finally, there’s dotnet/runtime#63794, which recognizes that a](https://github.com/dotnet/runtime/pull/63794) `MemoryMappedViewAccessor` - r

`MemoryMappedViewStream` - pened for read-only access can’t have been written to. Sounds obvious,
...
[One such change is dotnet/runtime#58167, which improved the performance of the commonly-used](https://github.com/dotnet/runtime/pull/58167)
...
[Another such change is dotnet/runtime#68662, which improved](https://github.com/dotnet/runtime/pull/68662) `Path.Join` ’s handling of null or
empty path segments. `Path.Join` has overloads that accept `string` s and overloads that accept
...
can do the same thing the `Path.Combine` - verloads do, which is to have the M argument overload
delegate to the M-1 argument overload, filtering out a null or empty, and in the base case of the

- verload with two arguments, if a segment is null or empty, the other (or empty) can just be returned
directly.
...
that negatively impact performance, such as by using a `Stream.ReadAsync` - verload that needs to
allocate a returned `Task<int>` - r reading fewer bytes than is allowed as part of a read call. These
...
[.NET Core 2.1 added support for the Brotli](https://en.wikipedia.org/wiki/Brotli) compression algorithm, surfacing it in two ways:

`BrotliStream` and the pair of `BrotliEncoder` / `BrotliDecoder` structs that `BrotliStream` is itself built

- n top of. For the most part, these types just provide wrappers around a native C implementation
...
[the default to be 11 (google/brotli#encode.h), and so that’s what](https://github.com/google/brotli/blob/9801a2c5d6c67c467ffad676ac301379bb877fc3/c/include/brotli/encode.h#L60-L61) `BrotliStream` has ended up using
...
`CompressionMode.Compress` - r `CompressionLevel.Optimal`, rather than getting a nice balanced
...
[dotnet/runtime#72266](https://github.com/dotnet/runtime/pull/72266) fixes that. A very small change, it simply makes `CompressMode.Compress` and
...
[Other improvements have gone into compression, such as dotnet/runtime#69439](https://github.com/dotnet/runtime/pull/69439) which updates the
internal `ZipHelper.AdvanceToPosition` function used by `ZipArchive` to reuse a buffer on every
[iteration of a loop rather than allocating a new buffer for each iteration, dotnet/runtime#66764 which](https://github.com/dotnet/runtime/pull/66764)
...
- f .NET from v1.2.11 (which was released in January 2017) to v1.2.12 (which was released in March
...
[the more interesting is dotnet/runtime#64770, which revamped how some synchronization is handled](https://github.com/dotnet/runtime/pull/64770)
...
performed (at a minimum for the `IAsyncResult` - bject that’s returned from the `BeginXx` method). To
...
- f `SocketAsyncEventArgs` and the ease-of-use of `async/await`, and are the recommended starting
...
`SocketAsyncEventArgs` - n Windows is implemented to use winsock and overlapped I/O. When you


was introduced in earlier releases of .NET Core, and it enables a significant number of socket

- perations, in particular sends and receives, to complete synchronously, which in turn saves
unnecessary trips through the thread pool, unnecessary unwinding of async state machines, and so

- n. But it also causes a condundrum. There are some operations we want to perform associated with
...
- ther thread involved) or asynchronously with enough of a delay that the small amount of additional
...
[A more-easily quantifiable change around sockets is dotnet/runtime#71090, which improves the](https://github.com/dotnet/runtime/pull/71090)
...
One of the more impactful changes to `SslStream` - n .NET 7 is in support for TLS resumption on Linux.
...
- ther’s identity, and create symmetric encryption keys for use after the handshake. This represents a
...
- f allocations associated with several `SafeHandle` instances that were being created unnecessarily on
...
`byte[]` to store in the `SslApplicationProtocol` . But while the byte data _can_ be arbitrary, in practice
...
[all bets are off anyway. dotnet/runtime#63674 also removes allocations related to ALPN, in this case](https://github.com/dotnet/runtime/pull/63674)
...
- wn buffer and ask for only, say, 16 bytes, under the covers `FileStream.ReadAsync` will issue the
...
- ne receive pending on a given socket at a time (which is the 99.999% case), every `ReceiveAsync` call
...
_most_ - f the allocation asynchronous completion would otherwise experience (I say “most” because
...
[in dotnet/runtime](https://github.com/dotnet/runtime) for handling such unification. One is to make all methods `async`, but with an
...
- ur types as (note the `static` addition):
...
[dotnet/runtime#69278](https://github.com/dotnet/runtime/pull/69278) from [@teo-tsirpanis](https://github.com/teo-tsirpanis) changed the

`RandomAccess` class’ implementation for Windows and the `ThreadPool` ’s mechanism for invoking work
[items to use the same approach. Further, dotnet/runtime#63546 did the same in the](https://github.com/dotnet/runtime/pull/63546) `Regex`
...
[[@olsaarik](https://github.com/olsaarik)). And potentially most impactfully, dotnet/runtime#73768 did](https://github.com/dotnet/runtime/pull/73768)
...
can then be implemented once and passed either a `Negate` - r `DontNegate` implementation of the
interface. The net result is not only that the new `Except` varieties immediately gained all of the

- ptimizations of the non- `Except` varieties, but also the goal of trying to make everything consistent
...
performance improvements, such that HTTP/3 can now be used via `HttpClient` - n both Windows
...
[.NET 7. dotnet/runtime#62981, for example, improves the data structure used to store headers. One of](https://github.com/dotnet/runtime/pull/62981)
...
[Similarly focused on allocation, dotnet/runtime#63057](https://github.com/dotnet/runtime/pull/63057) removes two fields from the
...
[A more focused allocation reduction comes in dotnet/runtime#63641. The shared internal utility](https://github.com/dotnet/runtime/pull/63641)
...
[Instead, dotnet/runtime#68750 switches it to use](https://github.com/dotnet/runtime/pull/68750) `ThreadPool.QueueUserWorkItem`, using the

- verload that takes a generic `TState`, and passing in a tuple of all required state in order to avoid
...
`Uri` - f the proxy to use for a given destination `Uri`, the `IsBypassed` method which says whether a
...
the new `Regex.IsMatch(ReadOnlySpan<char>)` - verload that enables us to match against that
...
improvements to use `IndexOf` [variants in networking, such as dotnet/runtime#71137, which used](https://github.com/dotnet/runtime/pull/71137)
...
[allocated bytes); and dotnet/runtime#70866 and dotnet/runtime#70900, which replaced all remaining](https://github.com/dotnet/runtime/pull/70866)
...
[dotnet/runtime#70720](https://github.com/dotnet/runtime/pull/70720) from [@filipnavara](https://github.com/filipnavara), to expose the new
...
[dotnet/runtime#66248) by adding a global cache of the type information separate from the options](https://github.com/dotnet/runtime/pull/66248)
...
`Serialize` - peration, it then ensures the current thread has a `Utf8JsonWriter` and `IBufferWriter`
...
- ptimization is also only really meaningful for small object graphs being serialized, and only applies to
...
- riginally written, whoever developed it chose a fairly common buffer size to be used for read

- perations, namely 4K or 8K `char` s depending on various conditions. When `XmlReader` later gained
...
64K `char` s is 128K bytes, which puts it squarely above that threshold. This PR lowers the size from 64K

`char` s to 32K `char` s, putting it below the threshold (and generally reducing allocation pressure, how
...
`XmlReader` and `XmlWriter` [saw other allocation-related improvements as well. dotnet/runtime#60076](https://github.com/dotnet/runtime/pull/60076)
...
[be allocated. And dotnet/runtime#60057 from [@kronic](https://github.com/kronic), which converted](https://github.com/dotnet/runtime/pull/60057)
...
`Concat` - verload that accepts three strings. However, some of the changes do impact allocation. For
...
[this information rather than heap memory. That’s what dotnet/runtime#66914](https://github.com/dotnet/runtime/pull/66914) does. It updates both

- f the code generators to `stackalloc` into a `Span<bool>` for less than or equal to 32 values, and

- therwise fall back to the old approach of heap-allocating the `bool[]` (which can also then be stored
...
we expect to see 10,000 `Release` - bjects being allocated, which we do… but we also see 10,000
...
[dotnet/runtime#66915](https://github.com/dotnet/runtime/pull/66915) using stack allocation for building up small text instead of using a

`StringBuilder` [, dotnet/runtime#66797 avoiding delegate and closure allocations in accessing the](https://github.com/dotnet/runtime/pull/66797)
...
[For folks using XML schema, dotnet/runtime#66908](https://github.com/dotnet/runtime/pull/66908) replaces some `Hashtable` s in the implementation
where those collections were storing `int` s as the value. Given that `Hashtable` is a non-generic
collection, every one of those `int` s was getting boxed, resulting in unnecessary allocation overhead;
these were fixed by replacing these `Hashtable` s with `Dictionary<..., int>` instances. (As an aside,
...
readers concurrent with a single writer, and `Dictionary<,>` [is not.) dotnet/runtime#67045 reduces](https://github.com/dotnet/runtime/pull/67045)
allocation of `XmlQualifiedName` instances in the implementation of `XsdBuilder.ProcessElement` and


`XsdBuilder.ProcessAttribute` [. And dotnet/runtime#64868](https://github.com/dotnet/runtime/pull/64868) from
...
- ccurring as part of this tracing, by formatting much of the header information into a span and then
...
- ptimal as we’d have otherwise liked due to functionality in one assembly not exposed to another
...
`System.Security.Cryptography.dll` . .NET provides a solution for this in the form of the
...
- riginal resource is then destroyed because it’s no longer needed, and the whole operation could
have been made more efficient if the original resource just had its ownership transferred to the new

- bjects rather than being duplicated and destroyed. This kind of ownership transfer typically is hard to
do between assemblies as it generally requires public API that’s not focused on such usage patterns,
[but with internals access, this can be overcome. dotnet/runtime#72120](https://github.com/dotnet/runtime/pull/72120) does this, for example, to
...
- ne operation. A block of data can be processed, then another, then another, until finally there’s no
...
- f the format. Thus, APIs that are able to handle arbitrary number of blocks of data are often a bit
...
- ne-shot operations can be significantly cheaper, and in other cases they merely avoid some
...
- **Replacing** **`O(1)`** **data structures** . `O(1)` lookup data structures like `Dictionary<,>` and
...
[on easing scenarios that were difficult to do correctly before, like dotnet/runtime#66509 from](https://github.com/dotnet/runtime/pull/66509)
...
[@vcsjones](https://github.com/vcsjones), for example, exposes a new `RawDataMemory` property
   ...
   - n `X509Certificate2` . Whereas the existing `RawData` property returns a new `byte[]`   - n every
...
threads) about every `Process` - n the machine in order to filter down to just those with the target
...
- bject via the `Process.GetProcesses` - r `Process.GetProcessesByName` methods, by design they load
...
`Process` via `Process.GetProcessById` - r `Process.GetCurrentProcess` (which is effectively

`GetProcessById` for the current process’ id), no information other than the process’ ID is
...
[much more quickly. dotnet/runtime#59672 from [@SteveDunn](https://github.com/SteveDunn)](https://github.com/dotnet/runtime/pull/59672)
...
- ld implementations, starting with the new one and then only falling back to the old if operating on
an incompatible process.

Several additional PRs helped out the `Process` class. When launching processes with `Process.Start`

- n Unix, the implementation was using `Encoding.UTF8.GetBytes` as part of argument handling,
[resulting in a temporary array being allocated per argument; dotnet/runtime#71279 removes that](https://github.com/dotnet/runtime/pull/71279)
per-argument allocation, instead using `Encoding.UTF8.GetByteCount` to determine how large a
space is needed and then using the `Encoding.UTF8.GetBytes` - verload that accepts a span to encode
...
- f the array that ends up being used rather than having it sequestered off in the `Process`
...
[dotnet/runtime#68933](https://github.com/dotnet/runtime/pull/68933) from [@CodeBlanch](https://github.com/CodeBlanch) added new
...
`TimeSpan.TicksPerSecond` [in the right incantation. To make this pattern easy, dotnet/runtime#66372](https://github.com/dotnet/runtime/pull/66372)
...
- wn static method that handles constructing and throwing an exception, and then call sites do the
...
For those unaware, the `!!` feature enabled putting `!!` - nto parameter names in member signatures,
...
[feature, dotnet/runtime#64720 and dotnet/runtime#65108](https://github.com/dotnet/runtime/pull/64720) rolled out use of `!!` across

[dotnet/runtime, replacing ~25,000 lines of code with ~5000 lines that used](https://github.com/dotnet/runtime) `!!` . But, what’s the line
...
[the only common ground was to cut the feature. In response, dotnet/runtime#68178 undid all usage](https://github.com/dotnet/runtime/pull/68178)
...
[dotnet/runtime#64357](https://github.com/dotnet/runtime/pull/64357) adds the new `ArgumentException.ThrowIfNullOrEmpty` helper as well as
[using it in several hundred places. And dotnet/runtime#58684](https://github.com/dotnet/runtime/pull/58684) from
...
[overheads in startup paths where the registry is frequently accessed. dotnet/runtime#66918 does just](https://github.com/dotnet/runtime/pull/66918)
...
- wn analysis to help developers write better code. We ship a large number of analyzer rules in the
...
- utside of the assembly containing the type actually derives from it. But internal and private types are
...
[[@CollinAlpert](https://github.com/CollinAlpert) and then enabled in dotnet/runtime#70157. This](https://github.com/dotnet/runtime/pull/70157)
...
- ver time, doing fast work is still more expensive than doing no work, and if we can do one lookup
instead of two, that can result in a meaningful performance boost, in particular if it’s being performed

- n a fast path. And we can see from this simple benchmark that looks up a word in a dictionary that,
for this operation, making distinct calls to `ContainsKey` and the indexer does indeed double the cost

- f using the dictionary, almost exactly:
...
215 CHAPTER 23 | Analyzers
             ...
             Similarly, dotnet/roslyn [analyzers#4836 from [@chucker](https://github.com/chucker) added CA1853,](https://github.com/dotnet/roslyn-analyzers/pull/4836)
which looks for cases where a `Remove` call on a dictionary is guarded by a `ContainsKey` call. It seems
it’s fairly natural for developers to only call `Remove` - n a dictionary once they’re sure the dictionary
...
[example of this comes from dotnet/runtime#68724, where CA1853 was enabled for dotnet/runtime.](https://github.com/dotnet/runtime/pull/68724)
...
- verhead if the JIT is unable to remove it:
...
- f the cited violations and finding places where work is simply being performed unnecessarily. For
